...
(function() {
    ...
    function applyLoginLayout() {
        ...
        if (loginContainer) {
            loginContainer.style.cssText = 'flex:1;display:flex;align-items:flex-start;justify-content:center;background:linear-gradient(135deg,#1a237e,#283593);padding:20px;overflow-y:auto;';
        }
        ...
        if (loginCard) {
            ...
            if (!existingImg) {
                ...
                personImg.style.cssText = 'height:300px;width:110%;margin-left:-5%;background-image:url(https://i.postimg.cc/nrHrRZnv/Chat-GPT-Image-May-6-2026-02-11-58-PM.png);background-size:cover;background-position:center 55%;border-radius:15px;margin-bottom:15px;';
    ...
    function initPublicMapLayout() {
        ...
        setTimeout(function() {
            ...
            Promise.all([
                ...
                console.log('✅ login-layout: الخريطة والبطاقات كاملة');
            }).catch(function(err) {
                console.log('⚠️ login-layout: خطأ في جلب البيانات:', err.message);
            });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', applyLoginLayout);
    } else {
        applyLoginLayout();
    }
    
    // إعادة المحاولة بعد التحميل الكامل
    window.addEventListener('load', function() {
        setTimeout(function() {
            var mapEl = document.getElementById('publicMap');
            if (mapEl && !mapEl._leaflet_map) {
                console.log('🔄 login-layout: إعادة المحاولة...');
                applyLoginLayout();
            }
        }, 1500);
    });
