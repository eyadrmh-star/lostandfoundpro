...
refreshAdminPanel = async function() {
    ...
    if (list) {
        list.innerHTML = reps.length === 0 ? '<p>لا توجد بلاغات معلقة</p>' : reps.map(report => `
            <div style="display:flex;justify-content:space-between;align-items:center;margin:8px 0;padding:10px;background:#fff3cd;border-radius:12px;">
                <div><strong>${report.type === 'lost' ? '🔴 Lost' : '🟢 Found'}</strong><br>${escapeHtml(report.desc)}<br><small>📍 ${report.city}</small></div>
                <div style="display:flex;gap:6px;">
                    <button class="approve-report-btn btn-sm btn-save" data-id="${report.id}">✅ Approve</button>
                    <button class="reject-report-btn btn-sm btn-red" data-id="${report.id}">❌ Reject</button>
                </div>
            </div>`).join('');
        document.querySelectorAll('.approve-report-btn').forEach(btn => {
            btn.onclick = async () => {
