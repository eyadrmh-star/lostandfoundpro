...
async function saveFound() {
    ...
    if (isAdmin) {
        ...
        if (typeof firebaseSaveFound !== 'undefined') { firebaseSaveFound(newItem); }
        saveToLocalStorage();
        showAlert(t('success'), t('reportSaved'));
    } else {
        db.collection('pendingReports').add(newItem);
...
async function refreshAdminPanel() {
    ...
    [...lostArray, ...foundArray].forEach(item => {
        ...
        else if (text.includes('vehicle')||text.includes('مركبة')) categories.vehicle++;
        else if (text.includes('animal')||text.includes('حيوان')) categories.animal++;
...
(async function patchAdmin() {
  ...
  // دالة جلب كل المستخدمين المعلقين من Firestore
  ...
  // دالة جلب كل المستخدمين المعتمدين (approved = true)
  ...
  // دالة تحديث قائمة الطلبات المعلقة (UI)
  function renderPendingList(pending) {
    ...
    listEl.innerHTML = pending.length === 0
      ...
      : pending.map(u => `
        <div style="display:flex;justify-content:space-between;align-items:center;margin:8px 0;padding:12px;background:var(--bg);border-radius:12px;flex-wrap:wrap;gap:8px;">
          <div><strong>${u.name}</strong><br><small>${u.email || u.phone}</small></div>
          <div style="display:flex;gap:6px;">
            <button class="approve-btn btn-sm btn-save" data-id="${u.id}">✅ ${t('approve')}</button>
            <button class="reject-btn btn-sm btn-red" data-id="${u.id}">❌ ${t('reject')}</button>
          </div>
        </div>
      `).join('');
  }

  // دالة تحديث قائمة المعتمدين (UI)
  function renderApprovedList(users) {
    ...
    listEl.innerHTML = approved.length === 0
      ? `<p style="color:var(--text-light);">${t('noItems')}</p>`
      : approved.map(u => `
        <div style="display:flex;justify-content:space-between;align-items:center;margin:5px 0;padding:10px;background:#e8f5e9;border-radius:12px;">
          <span>${u.name} (${u.email || u.phone})</span>
          <div style="display:flex;gap:6px;">
            <button class="view-user-btn btn-sm btn-teal" data-email="${u.email || u.phone}">👁️ ${t('details')}</button>
            <button class="send-msg-btn btn-sm btn-yellow" data-email="${u.email || u.phone}">📨 ${t('sendMessage')}</button>
            <button class="ban-user btn-sm ${u.banned ? 'btn-save' : 'btn-red'}" data-email="${u.email || u.phone}">${u.banned ? '✅ فك الحظر' : '🚫 ' + t('ban')}</button>
            <button class="delete-user-btn btn-sm btn-red" data-email="${u.email || u.phone}">🗑️ Delete</button>
          </div>
        </div>
      `).join('');
  }

  // ربط الأزرار بالوظائف الفعلية (توافق/ترفض/حظر/حذف)
  function bindAdminActions() {
    document.querySelectorAll('.approve-btn').forEach(btn => {
      btn.onclick = async () => {
        ...
        if (doc.exists) {
          ...
          await db.collection('users').add(data);
          await docRef.delete();
          refreshAdminPanel(); // تحديث اللوحة بالكامل
          showToast(t('userApproved'));
        }
      };
    });
    ...
    document.querySelectorAll('.delete-user-btn').forEach(btn => {
      btn.onclick = async () => {
        ...
        if (!confirm('⚠️ Delete user and all their reports permanently?')) return;
        const snap = await db.collection('users').where('email', '==', email).get();
        snap.forEach(async doc => await doc.ref.delete());
        // حذف تقاريره (يمكن تركه الآن)
        refreshAdminPanel();
        showToast('✅ User deleted', 'success');
      };
    });
  ...
  // حفظ الدالة الأصلية لوحة الإدارة
  const originalRefreshAdminPanel = refreshAdminPanel;

  // التعريف الجديد للوحة الإدارة
  refreshAdminPanel = async function () {
    if (!isAdmin) return;
    // استدعاء الأصلية لرسم الإحصائيات والرسوم (تبقى تعمل)
    originalRefreshAdminPanel();
    
    // جلب البيانات من Firestore وتحديث الأقسام المعلقة والمعتمدة
    const [pending, approvedUsers] = await Promise.all([
      getPendingUsers(),
      getApprovedUsers()
    ]);
    
    // تحديث العداد في الكارد
    const pendingCard = [...document.querySelectorAll('#adminDynamicContent > div:first-child > div')]
      .find(d => d.querySelector('small')?.innerText.includes('Pending') || d.querySelector('small')?.innerText.includes('قيد الانتظار'));
    if (pendingCard) {
      const num = pendingCard.querySelector('div[style*="font-size:32px"]');
      if (num) num.innerText = pending.length;
    }
    
    // رسم القوائم
    renderPendingList(pending);
    renderApprovedList(approvedUsers);
    bindAdminActions();
    
    // تحديث إجمالي المستخدمين
    const totalUsersEl = [...document.querySelectorAll('#adminDynamicContent > div:first-child > div')]
      .find(d => d.querySelector('small')?.innerText.includes('Total Users') || d.querySelector('small')?.innerText.includes('إجمالي المستخدمين'));
    if (totalUsersEl) {
      const num = totalUsersEl.querySelector('div[style*="font-size:32px"]');
      if (num) num.innerText = approvedUsers.length;
    }
  };

  // تصحيح دالة عرض تفاصيل المستخدم لتقرأ من Firestore
  const originalShowUserDetails = showUserDetails;
  showUserDetails = async function(email) {
    const snap = await db.collection('users').where('email', '==', email).get();
    if (snap.empty) {
      originalShowUserDetails(email); // fallback
      return;
    }
    const user = snap.docs[0].data();
    // باقي كود عرض التفاصيل كما هو، فقط تأكد من عرض بيانات user
    document.getElementById('adminPanel').classList.add('hidden');
    document.getElementById('userDetailsPage').classList.remove('hidden');
    ...
    const myLost = lostArray.filter(i => i.userEmail === email).length;
    const myFound = foundArray.filter(i => i.userEmail === email).length;
    container.innerHTML = `
      <div class="profile-card">
        ...
        <div style="margin-top:16px;display:flex;gap:8px;">
          ...
          <button id="sendUserMessageBtn" class="btn-sm btn-purple" data-email="${email}">${t('send')}</button>
        </div>
      </div>`;
    document.getElementById('sendUserMessageBtn')?.addEventListener('click', function() {
      const msg = document.getElementById('userMessageInput').value.trim();
      if (msg) {
