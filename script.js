...
var matchIcon = L.divIcon({className:'', html:"<div style='background:#8e44ad;color:white;border-radius:50%;width:42px;height:42px;text-align:center;line-height:42px;font-weight:bold;font-size:18px;box-shadow:0 3px 10px rgba(0,0,0,0.3);'>M</div>", iconSize:[42,42], popupAnchor:[0,-21]});
    ...
    Object.keys(coordGroups).forEach(key => {
        ...
        items.forEach((item, i) => {
            ...
            let imgTag = (item.images && item.images[0]) ? `<div style="text-align:center;"><img src="${item.images[0]}" style="max-width:100%;max-height:120px;border-radius:6px;margin-bottom:4px;display:block;margin-left:auto;margin-right:auto;"></div>` : '';
            marker.bindPopup(`<div style="font-size:14px;line-height:1.4;max-width:280px;max-height:400px;overflow-y:auto;padding:8px;">${imgTag}<b>${typeLabel}:</b> ${escapeHtml(item.desc)}<br><small style="font-size:12px;">👤 ${escapeHtml(item.name || 'Unknown')}<br>📍 ${item.city || 'N/A'}${item.country ? ', ' + item.country : ''}<br>📅 ${item.date || 'N/A'}<br>🕐 ${timeAgo(item.timestamp)}<br>🏷 ${item.category || 'other'}</small>${isReward ? '<br><span style="color:#f0a500;font-weight:bold;font-size:16px;">💰 $' + item.reward.moneyAmount + '</span>' : ''}${isMatched ? '<br><span style="color:#8e44ad;font-weight:bold;font-size:16px;">🎯 Matched!</span>' : ''}<br><small style="font-size:11px;">👁️ ${reportViews[item.id] || 0} views</small></div>`);
        });
    });
    ...
    Object.keys(usedCoords).forEach(key => {
        ...
        items.forEach((item, i) => {
            ...
            L.marker([baseLat + offsetLat, baseLng + offsetLng], {icon: icon})
                .addTo(publicMap)
                .bindPopup('<div style="font-size:14px;line-height:1.4;max-width:280px;max-height:400px;overflow-y:auto;padding:8px;">' + (item.images && item.images[0] ? '<div style="text-align:center;"><img src="' + item.images[0] + '" style="max-width:100%;max-height:100px;border-radius:6px;margin-bottom:4px;display:block;margin-left:auto;margin-right:auto;"></div>' : '') + '<b>' + (item.itemType === 'lost' ? '🔴 Lost' : '✅ Found') + ':</b> ' + escapeHtml(item.desc) + '<br><small style="font-size:12px;">👤 ' + escapeHtml(item.name || 'Unknown') + '<br>📍 ' + (item.city || 'N/A') + (item.country ? ', ' + item.country : '') + '<br>📅 ' + (item.date || 'N/A') + '<br>🕐 ' + timeAgo(item.timestamp) + '<br>🏷 ' + (item.category || 'other') + '</small>' + (item.reward?.money ? '<br><span style="color:#f0a500;font-weight:bold;font-size:14px;">💰 $' + item.reward.moneyAmount + '</span>' : '') + '<br><small style="font-size:11px;">👁️ ' + (reportViews[item.id] || 0) + ' views</small></div>');
        });
    });
...
function attachAppEvents() {
    ...
    document.getElementById('lostCurrentLocationBtn')?.addEventListener('click',()=>{if(lostSelectMap){let c=lostSelectMap.getCenter();if(lostMarker)lostSelectMap.removeLayer(lostMarker);lostMarker=L.marker([c.lat,c.lng]).addTo(lostSelectMap);document.getElementById('lostLat').value=c.lat.toFixed(6);document.getElementById('lostLng').value=c.lng.toFixed(6);}});
    document.getElementById('foundCurrentLocationBtn')?.addEventListener('click',()=>{if(foundSelectMap){let c=foundSelectMap.getCenter();if(foundMarker)foundSelectMap.removeLayer(foundMarker);foundMarker=L.marker([c.lat,c.lng]).addTo(foundSelectMap);document.getElementById('foundLat').value=c.lat.toFixed(6);document.getElementById('foundLng').value=c.lng.toFixed(6);}});
...
updateDashboardMap = async function() {
   ...
   // ==========================================
//  نظام النوافذ المنبثقة للخريطة (بدل popup) - النسخة النهائية
// ==========================================
...
// ===== تعديل دالة updateDashboardMap مع الحفاظ على الدالة الأصلية =====
let originalUpdateDashboardMap = window.updateDashboardMap;

window.updateDashboardMap = async function() {
    // تشغيل الدالة الأصلية
    if (typeof originalUpdateDashboardMap === 'function') {
        await originalUpdateDashboardMap();
    }
    ...
    setTimeout(function() {
        document.querySelectorAll('#dashboardMap .leaflet-marker-icon').forEach(function(marker) {
            if (!marker._listenerAdded) {
                marker._listenerAdded = true;
